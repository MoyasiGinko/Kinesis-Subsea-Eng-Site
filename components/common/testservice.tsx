...
export default function ServicesSection() {
  ...
  const services = [
    ...
    },
    {
      ...
      description:
        "Innovative solutions for sustainable energy development and management.",
      image:
  ...
  return (
    <section
      ref={sectionRef}
      className="relative py-24 px-4 bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 overflow-hidden min-h-[600px]"
    >
      {/* Background Effects */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto flex flex-col lg:flex-row gap-12">
        {/* Services Grid Left */}
        <div className="lg:w-2/3 w-full grid grid-cols-1 md:grid-cols-2 gap-8 order-1 lg:order-none">
          {services.map((service, index) => (
            <div
              key={index}
              className={`group relative overflow-hidden rounded-2xl bg-slate-900 shadow-lg hover:shadow-2xl transition-all duration-700 hover:-translate-y-1 cursor-pointer min-h-[260px] flex flex-col items-center justify-center p-8 ${
                isVisible
            ...
            >
              <div className={`flex flex-col items-center justify-center mb-4`}>
                <div
                  className={`p-4 rounded-xl bg-gradient-to-r ${service.color} text-white shadow-lg mb-2`}
                >
                  {service.icon}
                </div>
                <h3 className="text-2xl font-bold text-white mb-2 text-center group-hover:text-slate-100 transition-colors duration-300">
                  {service.title}
                </h3>
                <p className="text-slate-300 text-base text-center mb-4 group-hover:text-slate-200 transition-colors duration-300 max-w-xs">
                  {service.description}
                </p>
                <button
        ...
        {/* Sticky Header Right */}
        <div className="lg:w-1/3 w-full lg:sticky lg:top-32 self-start order-2 lg:order-none text-right">
          <div
          ...
          <h2
            className={`text-4xl md:text-5xl font-light text-white mb-4 leading-tight transition-all duration-1000 delay-200 ${
              isVisible
                ? "opacity-100 translate-y-0"
                : "opacity-0 translate-y-8"
            } text-right`}
          ...
          <p className="text-lg text-slate-300 max-w-md font-light transition-all duration-1000 delay-300 text-right">
